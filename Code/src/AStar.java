...
public class AStar {
    ...
    public ISearchNode search(ISearchNode initialNode, IGoalNode goalNode) {
        ...
        this.bestNodeAfterSearch = min(closedSet);
        return null;
    }

    public ISearchNode min(ArrayList<ISearchNode> queue) {
      ISearchNode smallestNode = null;
      for(ISearchNode node : queue) {
         if(node.compareTo(smallestNode) == -1) { //is less than
            smallestNode = node;
         }
      }
      return smallestNode;
     ...
     */
    private static ISearchNode getNode(PriorityQueue queue, ISearchNode searchedNode) {
